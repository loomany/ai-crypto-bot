from typing import Dict

COIN_INFO: Dict[str, str] = {
    "BTC": (
        "Bitcoin — первая и самая известная криптовалюта. "
        "Используется как цифровое \"золото\" и средство сохранения стоимости. "
        "Запущен в 2009 году анонимным разработчиком под псевдонимом Satoshi Nakamoto."
    ),
    "ETH": (
        "Ethereum — блокчейн для смарт-контрактов и децентрализованных приложений (dApp). "
        "Монета ETH используется для оплаты комиссий и как основная валюта в экосистеме DeFi и NFT."
    ),
    "LAZIO": (
        "LAZIO Fan Token — токен болельщиков футбольного клуба «Лацио». "
        "Даёт доступ к фан-голосованиям и акциям через платформу Socios и биржу Binance. "
        "Торгуется как обычная криптовалюта на спотовом рынке."
    ),
    "BTTC": (
        "BTTC (BitTorrent Chain) — токен экосистемы BitTorrent и Tron. "
        "Используется для стейкинга, комиссий и взаимодействия с кроссчейн-мостом BitTorrent Chain. "
        "Торгуется на Binance как низкоценная монета с большой эмиссией."
    ),
    # сюда можно позже добавлять новые монеты по мере надобности
}


def get_coin_description(symbol: str) -> str:
    """
    symbol может быть 'BTCUSDT', 'LAZIOUSDT' и т.п.
    Возвращает короткое описание монеты или дефолтный текст.
    """
    base = symbol.replace("USDT", "").replace("BUSD", "").replace("USDC", "")
    base = base.upper()

    desc = COIN_INFO.get(base)
    if desc:
        return desc

    return (
        f"{base} — торговая пара на Binance. "
        f"Пока подробное описание не добавлено, "
        "но монету можно анализировать по графику, объёмам и тех.индикаторам."
    )
